  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: ()async{
          var userCreate = UserCreate(username.text, email.text, sex.name, isactive?'active':'inactive');
          print(userCreate.toString());
          // print(userCreate.gender);
          var userRespone = await APIService().createUser(userCreate);
          Navigator.of(context).push(MaterialPageRoute(builder: (context)=>Page3(userRespone)));
        },
        child: Text('Regist'),
      ),
      appBar: AppBar(
      ...
      body: Form(child: Column(
        children: [
          ...
          ListTile(
            title: Text('Male'),
            leading: Radio(value:Sex.male , groupValue: sex, onChanged: (Sex?value){
              setState(() {
                sex = value!;
              });
            }),
            ...
            title: Text('Female'),
            leading: Radio(value:Sex.female , groupValue: sex, onChanged: (Sex?value){
              setState(() {
                sex = value!;
              });
            }),
